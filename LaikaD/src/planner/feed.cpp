...
bool Feed_Class::drop_and_weigh(int16_t meal_qt_gr)
{
	...
	while (true)
	{
		...
		if ((current_millis - last_millis_load_cell_time_ceck) > LOAD_CELL_TIME_CECK)
		{
			...
			display.food_val(currently_weight, meal_qt_gr, total_currently_weight+currently_weight, total_target_weight);
...
int16_t Feed_Class::get_total_currently_weight(){
	return total_currently_weight;
}