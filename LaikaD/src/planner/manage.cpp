...
void Manage_Class::reset_day() {
	...
	for (int i = 0; i < 4; i++) { //resetta i pasti erogati
		done_meal[i] = false;
	}
}
...
void Manage_Class::main_function() {
	...
	if (daily_ceck_to_do) { //se è vero il calcolo giornaliero dei valori e ancora da eseguire
	...
	if (!today_all_done && !error_occur_in_feed) { //se c'è un errore in erogazione viene bloccato il ciclo di erogazione
		
		if (its_the_moment() && food_left_in_tank()) { //controlla se è l'ora giusta e quale pasto si deve erogare aggiornando l'indice index_of_this_meal ed il cibo rimasto nel serbatoio
			...
			if (done_meal[index_of_this_meal]) {//se il pasto è stato erogato correttamente

				if (index_of_this_meal < n_of_meals) {//calcolo adj_gr_meal del prossimo pasto
